...
var insertItem = function(wildStream) {
  ...
  wildStream.attach(newItem.querySelector('.video'));
...
function itemBindHover(item) {
  ...
  item.querySelector('.camera-on').addEventListener('click', function() {
    ...
    list.children[0].querySelector('.video').style.display = 'none';
  ...
  item.querySelector('.camera-off').addEventListener('click', function() {
    ...
    list.children[0].querySelector('.video').style.display = 'inline-block';
    var uid = auth.currentUser.uid;
  ...
  .then(function(user) {
    ...
    videoInstance.createStream({
      ...
      .then(function(wdStream) {
        ...
        localStream.attach('#localStream');
        ...
        ref.child('isVideo/' + user.uid).set(true);
        ref.child('isVideo/' + user.uid).onDisconnect().remove();
        var conference = client.connectToConference(roomId, {
          stream: localStream,
          userData: 'wildrtc'
        });
        conference.on('participant_connected', function(participant) {
          participant.on('streamAdded', function(stream) {
            remoteList.push(participant.Id);
            insertItem(stream);
            ref.child('isVideo/' + participant.Id).on('value', function(snap) {
              var removeIndex = remoteList.indexOf(participant.Id);
              if (snap.val() === false) {
